...
function parseEventTag(tag: string, id: number): PToken {
  if (tag.startsWith('{/')) {
    ...
    return {
      type: 'event',
      eventType: eventNameTags[tagType],
...
const eventNameTags: { [x: string]: PrompterEvent['event'] } = {
  edit: 'switchSection',
  image: 'switchToImage',
...
// #region DEBUG HELPERS - TESTING ONLY

// creates a ton of text, with occasional section switches
...
// short exerpt from the sample script
